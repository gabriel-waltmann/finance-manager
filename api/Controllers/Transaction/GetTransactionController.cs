...
) : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<GetTransactionResponse>> ExecuteAsync([FromRoute] string id)
    {
        try
        {
            var transaction = await _service.GetWithTransactionPerson(Guid.Parse(id));
